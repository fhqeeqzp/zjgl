...
def test_import():
    ...
    for row_idx, row in enumerate(sheet.iter_rows(min_row=start_row, values_only=True), start=start_row):
        # 检测表头行 - 跳过但不保存pending_item（让续行能够合并）
    ...
    print("前25条数据预览:")
    print("-" * 100)
    for i, item in enumerate(imported_items[:25], 1):
    ...
    for item in imported_items:
        if '现浇混凝土基础 独立基础 混凝土' in item.get('name', ''):
            print(f"\n✓ 找到合并后的项目: {item['name']}")
            print(f"  项目特征: {item['description'][:100]}...")
            found = True
            break
